...
class Select {
      ...
      do {
        ...
        while(n!=null && newgroup==false) {
          ...
          for(int i=0;i<iGroupLen;i++) {
            if(n.next==null) {
              newgroup=true;
            } else if(Column.compare(n.data[i],n.next.data[i],r.iType[i])!=0) {
  ...
  private void addAggregateRow(Result x,Object row[],int len,int count)
  throws SQLException {
    for(int i=0;i<len;i++) {
  ...
  private static Result removeSecond(Result r,Result minus)
  throws SQLException {
    int len=r.getColumnCount();
    Record n=r.rRoot;
    Record last=r.rRoot;
  ...
  private static Result removeDifferent(Result r,Result r2)
  throws SQLException {
    int len=r.getColumnCount();
    Record n=r.rRoot;
    Record last=r.rRoot;
  ...
  throws SQLException {
    ...
    for(int blocksize=1;target[1]!=null;blocksize<<=1) {
      source0=target[0];source1=target[1];
      ...
      for(dest=0;source0!=null;dest^=1) {
        ...
        while(true) {
          if(n0==0 || source0==null) {
            ...
            n=source1;source1=source1.next;n1--;
          } else if(n1==0 || source1==null) {
            n=source0;source0=source0.next;n0--;
          } else if(compareRecord(source0.data,source1.data,r,order,way)>0) {
            n=source1;source1=source1.next;n1--;
          } else {
            n=source0;source0=source0.next;n0--;
          }
  ...
  int order[],int way[]) throws SQLException {
