...
class Index {
  ...
  Node findFirst(Object value,int compare) throws SQLException {
    Trace.assert(compare==Expression.BIGGER || compare==Expression.EQUAL ||
    compare==Expression.BIGGER_EQUAL,"Index.findFirst");
    Node x=root;
    int iTest=1;
    ...
    while(x!=null && compareValue(value,x.getData()[iColumn_0])>=iTest) {
      if(Trace.STOP) Trace.stop();
      x=next(x);
    }
    return x;
  ...
  private boolean from(Node x) throws SQLException {
    ...
    if(Trace.ASSERT) Trace.assert(x.getParent()!=null);
