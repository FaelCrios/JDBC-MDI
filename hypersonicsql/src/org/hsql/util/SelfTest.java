...
class SelfTest {
  ...
  static void test(int max,boolean persistent) {
    ...
    try {
      ...
      if(persistent) {
        ...
        delete("test2.script");
        delete("test2.data");
        test("Persistent","jdbc:HypersonicSQL:test2","sa","",max,true);
      }
      test("In-Memory","jdbc:HypersonicSQL:.","sa","",max,false);
  ...
  static void test(String name,String url,String user,String password,int max,
  boolean persistent) throws Exception {
    ...
    try {
      ...
      // prepared statements
      {
        ...
        r=sStatement.executeQuery(s);
        r.next();
        if(!r.getString(2).equals("\"Birdie\"'s car \ufedc")) {
