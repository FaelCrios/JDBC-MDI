...
implements WindowListener,ActionListener,ItemListener {
  ...
  void main() {
    ...
    String extras[]={"Insert 10 rows only","Insert 1000 rows only",
    "Insert all rows"};
    ...
    try {
      ...
      while(result.next()) {
        hTypes.put(new Integer(result.getShort(2)),result.getString(1));
      ...
      for(int i=0;i<tTable.size();i++) {
        ...
        ResultSet col=dSourceMeta.getColumns(null,null,t.sSourceTable,null);
        ...
        while(col.next()) {
          ...
          if(datatype==null) {
            datatype=source;
            trace("No mapping for type: "+inttype+" source type is "+datatype);
          }
  ...
  void transfer() {
    ...
    try {
      ...
      trace("Transfer stopped - "+last+" / "+sLast+" / Error: "+e.getMessage());
      e.printStackTrace();
    } catch(Exception e) {
      String last=tMessage.getText();
      trace("Transfer stopped - "+last+" / "+sLast+" / Error: "+e.getMessage());
  ...
  void transfer(Table t) throws SQLException {
    ...
    if(t.bInsert) {
      ...
      ResultSet r=sSourceStatement.executeQuery(t.sSourceSelect);
      int i=0;
  ...
  String sLast;
  void transferRow(int type[],ResultSet r,PreparedStatement p)
  throws SQLException {
    sLast=null;
    p.clearParameters();
    int len=type.length;
